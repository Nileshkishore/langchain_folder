...
from custom_ollama import OllamaLLMWithMetadata  # Your Llama 3.2 model
...
llm = OllamaLLMWithMetadata(model="llama3.2")

# Function to log MLflow asynchronously
def log_to_mlflow(user_input, prompt, result, retrieved_docs, cosine_score):
...
while True:
    ...
    # **2️⃣ Then show retrieved document info**
    if retrieved_docs_with_scores:
    ...
    # **3️⃣ Log MLflow in the background (non-blocking)**
    thread = threading.Thread(target=log_to_mlflow, args=(user_input, prompt, result, [top_doc] if top_doc else [], cosine_score))
    thread.daemon = True  # Ensures thread does not prevent exit
    thread.start()
